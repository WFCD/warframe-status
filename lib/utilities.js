...
const kuvaCache = new Cache('https://10o.io/kuvalog.json', 300000, { logger, delayStart: true });
const sentientCache = new Cache('https://semlar.com/anomaly.json', 300000, { logger, delayStart: true });
...
platforms.forEach((p) => {
  ...
  warframeData.locales.forEach((locale) => {
    worldStates[p][locale] = new WSCache(p, locale, kuvaCache, sentientCache);
  });
