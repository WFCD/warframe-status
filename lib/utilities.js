...
const kuvaCache = new Cache('https://10o.io/kuvalog.json', 300000, { useEmitter: false, logger: logger, delayStart: false, maxRetry: 1 });
...
platforms.forEach((p) => {
  ...
  warframeData.locales.forEach((locale) => {
    worldStates[p][locale] = new Cache(url, wsTimeout, {
      parser, logger: logger, delayStart: !(p === 'pc' && locale === 'en'), opts: { locale, kuvaCache },
    });
  });
});
