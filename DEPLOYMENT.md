...
- **Docker image job** — builds the image, runs it behind WARP the same way as `docker-compose.warp.example.yml`, then checks `/heartbeat` and `/factions` (static bundled data) from inside the WARP network namespace (`127.0.0.1:3001`). CI does not smoke-test live worldstate — that path is covered by the test job. CI avoids curling published host ports because WARP can intercept Docker bridge traffic; set `BETA_FIX_HOST_CONNECTIVITY=1` on the WARP container if you need host access in production.
...
If WARP registration fails in CI, check `docker logs warp` in the job output. First connect can take up to ~60s.
