...
router.get('/', cache('1 minute'), (_req, res) => {
  res.json({ code: 200, message: 'OK' });
});

router.use((req, res, next) => {
  req.platform = (
    req.url.replace('/', '').trim().split('/')[0] ||
  ...
  if (req.header('Accept-Language')?.length)
    res?.set('Vary', 'Accept-Language');

  req.language = (req.header('Accept-Language') || 'en')
    .substring(0, 2)
    .toLowerCase();
...
router.use(
  ['warframes', 'weapons', 'items', 'mods'].flatMap((itype) => [
    `/${itype}`,
    `/${itype}/`,
  ]),
  items,
