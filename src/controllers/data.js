    ...
    try {
      queries.forEach((q) => {
        ...
        switch (req.key) {
          ...
          case 'solNodes': {
            keyResults = solKeys.filter((solNodeKey) =>
              solNodeKey.toLowerCase().includes(loweredQuery),
            );
            solKeys.forEach((solKey) => {
              if (
                warframeData[req.language].solNodes[solKey] &&
                warframeData[req.language].solNodes[solKey].value
                  .toLowerCase()
          ...
          default:
            Object.keys(
              warframeData[req.language][req.key] || warframeData[req.key],
            ).forEach((selectedDataKey) => {
              if (selectedDataKey.toLowerCase().includes(loweredQuery)) {
                results.push(
                  (warframeData[req.language][req.key] ||
                    warframeData[req.key])[selectedDataKey],
                );
              }
            });
            value = results;
