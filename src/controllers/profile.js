...
const get = async (id) => {
  ...
  const data = await fetch(profileUrl, {
    headers: { 'User-Agent': process.env.USER_AGENT || 'Node.js Fetch' },
  });
  ...
  return res
    ...
    .json(
      new Profile(
        profile.Results[0],
        req.language,
        req.query.withItem || false,
      ),
    );
});

router.get(
  ['/:playerId/xpInfo/', '/:playerId/xpInfo'],
  cache('1 hour'),
  async (req, res) => {
    ...
    const xpInfo = data.Results[0].LoadOutInventory.XPInfo.map(
      (xp) => new XpInfo(xp, req.query.withItem || false),
    );
...
router.get(
  ['/:username/arsenal/', '/:username/arsenal'],
  cache('1 hour'),
  async (req, res) => {
