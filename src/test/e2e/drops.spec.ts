...
describe('drops', () => {
  ...
  it('should be an array', async () => {
    const res = await request.execute(nestApp.getHttpServer()).get('/drops');
  ...
  describe('search', () => {
    ...
    describe('by place', () => {
      it('should look up hydron', async () => {
        const res = await request
          .execute(nestApp.getHttpServer())
          .get('/drops/search/Sedna%2FHydron (Defense), Rot A');
        res.should.have.status(200);
        res.body.should.be.an('array');
        res.body.length.should.be.greaterThan(0);
      });
      ...
      it('should be case insensitive', async () => {
        const res = await request
          .execute(nestApp.getHttpServer())
          .get('/drops/search/sedna%2Fhydron (defense)');
        res.should.have.status(200);
        res.body.should.be.an('array');
        res.body.length.should.be.greaterThan(0);
        res.body
    ...
    it('should group by location', async () => {
      const res = await request
        .execute(nestApp.getHttpServer())
        .get('/drops/search/forma?grouped_by=location');
      res.should.have.status(200);
    ...
    it('should respond with an empty object for no results ', async () => {
      const res = await request
        .execute(nestApp.getHttpServer())
        .get('/drops/search/lephantisxda?grouped_by=location');
      res.should.have.status(200);
      res.body.should.be.an('Object');
